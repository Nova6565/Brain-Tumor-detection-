 ...
 * Generated at: 2024-05-15 19:11:29 UTC
  ...
  public void _jspService(final jakarta.servlet.http.HttpServletRequest request, final jakarta.servlet.http.HttpServletResponse response)
    ...
    try {
      ...
      synchronized (session) {
        u = (advanced.User) _jspx_page_context.getAttribute("u", jakarta.servlet.jsp.PageContext.SESSION_SCOPE);
        if (u == null){
          u = new advanced.User();
          _jspx_page_context.setAttribute("u", u, jakarta.servlet.jsp.PageContext.SESSION_SCOPE);
          out.write('\r');
          out.write('\n');
      ...
      out.write("	");
				...
				u.setPassword(password);
				    if(db.emailFound(email)){
				        if(db.Login(u)!=null) {
				           u=db.Login(u);
				            // Set cookies
				            Cookie firstnameCookie = new Cookie("firstname", u.getFirstname().trim());
				            Cookie emailCookie = new Cookie("email", u.getEmail().trim());
				            Cookie passwordCookie = new Cookie("password", u.getPassword());
				            //Cookie userIDCookie = new Cookie("User Id",""+u.getId()+"");
				            if(remember) {
				                firstnameCookie.setMaxAge(1000000);
				                emailCookie.setMaxAge(100000000);
				                passwordCookie.setMaxAge(100000000);
				                //userIDCookie.setMaxAge(100000000);
				            }
				            response.addCookie(firstnameCookie);
				            response.addCookie(emailCookie);
				            response.addCookie(passwordCookie);
				            //response.addCookie(userIDCookie);
							//ut.print("<p class='success-msg'>Log in succesfully</p>");
				            request.getRequestDispatcher("Homaepage.jsp").include(request, response);
				        }
				        
				        else{
				        	out.print("<p class='error-msg'>Wrong password</p>");
				            request.getRequestDispatcher("Index.jsp").include(request, response);
				        }
				    }
				    else{
				    	out.print("<p class='error-msg'>Wrong email</p>");
			            request.getRequestDispatcher("Index.jsp").include(request, response);
				    }
